...
const { src, dest } = require("gulp"),
  ...
  // webpcss = require("gulp-webpcss"), // // Class add for webp - image and no-webp image
  webpCss = require("gulp-webp-css"),
  // svgSprite = require("gulp-svg-sprite"); // For Sprite
...
function css() {
  return (
    ...
    .pipe(group_media())
    // // Class add for webp - image and no-webp image
...
function fontsStyle(params) {
  let file_content = fs.readFileSync(source_folder + "/scss/fonts/_fonts.scss");
  if (file_content == "") {
    fs.writeFile(source_folder + "/scss/fonts/_fonts.scss", "", cb);
      ...
      if (items) {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (c_fontname != fontname) {
            fs.appendFile(
              source_folder + "/scss/fonts/_fonts.scss",
              '@include font("' +
              fontname +
              '", "' +
              fontname +
